...
$Description = $_POST['Description'];
  ...
  if (!empty($Nom_Catégories) && !empty($Description) ) {
    ...
    if (!empty($uploadedFile)) {
      ...
      $categories = new categ("",$Nom_Catégories,$Description ,$uploadedFile , NULL) ;  
            ...
            <table class="table table-striped table-hover " >
                ...
                <tbody class="table-group-divider">
                  <?php foreach ($categorieData as  $value) {   ?>
                    ...
                    <tr>
                    <td ><img src="<?= $value->get_catg_img() ?>" alt="" width="150px" height="154px"></td>
                    <th  scope="row"><?= $value->getNom() ?></th>
                    <td ><?= $value->get_catg_Description() ?></td>
                    ...
                    <button onclick="NoneRequest(<?= $value->getId() ?>, this)" class="btn btn-<?php if (is_null($value->getdeleted_at_catg())) {
                      echo "info" ;
                    }else {
                      echo "secondary" ;
                    } ?> mb-2 ms-2" type="button" ><div id="result_<?= $value->getId() ?>"><?php if (is_null($value->getdeleted_at_catg())) {
                      echo "None" ;
                    }else {
                      echo "Block" ;
                    } ?></div></button>
